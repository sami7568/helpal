...
class HelpeeOtpScreen extends StatefulWidget {
  final String phonNumber;

  const HelpeeOtpScreen({Key key, this.phonNumber}) : super(key: key);

  @override
  _HelpeeOtpScreenState createState() =>
      new _HelpeeOtpScreenState(this.phonNumber);
}

class _HelpeeOtpScreenState extends State<HelpeeOtpScreen>
    with SingleTickerProviderStateMixin {
  final phoneNumber;
  _HelpeeOtpScreenState(this.phoneNumber);
  // Constants
  final int time = 60;
...
//        padding: new EdgeInsets.only(bottom: 16.0),
  ...
  void otpEntered(String otp) async {
    ...
    if (isCorrect == true) {
      ...
      dynamic isExist =
          await AuthService().ifDetailsExists('helpees', phoneNumber);
      print("user is=$isExist");
      //if exist
      if (isExist == true) {
        await AuthService().saveDefaultLocalKeys(phoneNumber, 'helpees');
      ...
      //if not exist in database
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => HelpeeSignup(phoneNumber: phoneNumber),
          ),
        );
      } else {
