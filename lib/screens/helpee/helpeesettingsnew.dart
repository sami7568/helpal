...
class HelpeeSettings extends StatefulWidget {
  @override
  _HelpeeSettingsState createState() => _HelpeeSettingsState();
}

class _HelpeeSettingsState extends State<HelpeeSettings>
  ...
  Future getKeepMeSignedin() async {
  ...
  void callbackFunction(String callFrom) {
    ...
    if (callFrom.contains('account')) {
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => HelpeeSettingsAccount()));
    }
  }
  ...
  List<Widget> getListItem() {
    ...
    listitem.forEach((key, value) {
      ...
      if (key.contains('empty')) {
        listofwidgets.add(new SizedBox(height: 5));
      } else {
